...
import type { TestCasesFilters } from '../types';

const STATUS_OPTIONS = ['Draft', 'Ready', 'Passed', 'Failed', 'Blocked'];
    ...
    <div className="p-6 max-w-7xl mx-auto">
      <div className="flex items-center justify-between mb-6">
        <div>
          ...
          <p className="text-sm text-gray-500 mt-1">Traceability between requirements and tests</p>
        </div>
        <Link
          to="/test-cases/new"
        ...
        <table className="w-full text-sm">
          <thead className="bg-gray-50 border-b border-gray-200">
            <tr>
              {[
                ...
                { label: 'Linked Requirement', field: null },
          ...
          <tbody className="divide-y divide-gray-100">
            {isLoading ? (
              ...
              <tr><td colSpan={5} className="px-4 py-8 text-center text-gray-400">No test cases found</td></tr>
            ) : data.items.map(tc => (
              <tr
                key={tc.id}
              ...
              >
                <td className="px-4 py-3 font-medium text-gray-900 max-w-xs truncate">{tc.title}</td>
                <td className="px-4 py-3">
                  <span className={`inline-flex items-center px-2 py-0.5 rounded-full text-xs font-medium border ${STATUS_COLORS[tc.status] ?? 'bg-gray-100 text-gray-600'}`}>
                    {tc.status}
                  </span>
                </td>
                <td className="px-4 py-3 text-gray-500">
                  {tc.requirement ? (
                    <span className="text-indigo-600">{tc.requirement.req_id}</span>
                  ) : '—'}
                </td>
                <td className="px-4 py-3 text-gray-500">{new Date(tc.updated_at).toLocaleDateString()}</td>
                <td className="px-4 py-3 text-right">
                  <button
                    onClick={e => { e.stopPropagation(); setDeleteConfirm(tc.id); }}
