...
import { useAssumptions, useDeleteAssumption } from '../hooks/useAssumptions';
import type { AssumptionsFilters } from '../types';
...
type SortField = 'title' | 'status' | 'priority' | 'category' | 'updated_at';
...
export default function AssumptionsListPage() {
  ...
  const filters: AssumptionsFilters = {
  ...
  const { data, isLoading } = useAssumptions(filters);
  ...
  return (
    <div className="p-6 max-w-7xl mx-auto">
      <div className="flex items-center justify-between mb-6">
        <div>
          <h1 className="text-2xl font-bold text-gray-900">Assumptions & Unknowns</h1>
          <p className="text-sm text-gray-500 mt-1">Track assumptions, open questions, and risks</p>
        </div>
        <Link
          to="/assumptions/new"
      ...
      <div className="flex gap-3 mb-4 flex-wrap">
        ...
        />
        <select value={category} onChange={e => { setCategory(e.target.value); setPage(1); }} className="px-3 py-2 border border-gray-300 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-indigo-500">
        ...
        <select value={priority} onChange={e => { setPriority(e.target.value); setPage(1); }} className="px-3 py-2 border border-gray-300 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-indigo-500">
        ...
        <table className="w-full text-sm">
          ...
          <tbody className="divide-y divide-gray-100">
            {isLoading ? (
              ...
              <tr key={a.id} className="hover:bg-gray-50 cursor-pointer" onClick={() => navigate(`/assumptions/${a.id}`)}>
                ...
                <td className="px-4 py-3">
                  <span className={`inline-flex items-center px-2 py-0.5 rounded-full text-xs font-medium border ${STATUS_COLORS[a.status] ?? 'bg-gray-100 text-gray-600'}`}>
                    {a.status}
                  </span>
                </td>
                <td className="px-4 py-3">
                  <span className={`inline-flex items-center px-2 py-0.5 rounded-full text-xs font-medium border ${PRIORITY_COLORS[a.priority] ?? 'bg-gray-100 text-gray-600'}`}>
                    {a.priority}
                  </span>
                </td>
                <td className="px-4 py-3 text-gray-500">{a.owner?.name ?? '—'}</td>
                <td className="px-4 py-3 text-gray-500">{new Date(a.updated_at).toLocaleDateString()}</td>
