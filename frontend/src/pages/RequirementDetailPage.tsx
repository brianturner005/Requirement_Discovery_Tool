    ...
    <div className="p-8 space-y-6 max-w-5xl">
      ...
      <div className="flex items-center justify-between flex-wrap gap-3">
        <div className="flex items-center gap-2 text-sm text-slate-500">
          ...
          <span className="font-mono font-medium text-slate-700">{requirement.req_id}</span>
        </div>
          ...
          <div className="mt-5 pt-4 border-t border-slate-100">
            ...
            <div className="flex flex-wrap gap-1.5">
              {requirement.tags.map((tag) => (
                <span
                  key={tag.id}
                  className="px-2 py-0.5 bg-indigo-50 text-indigo-700 text-xs rounded-full font-medium border border-indigo-100"
                >
                  {tag.name}
                </span>
              ))}
            ...
            {requirement.related_requirements.map((rel) => (
              ...
              >
                <div className="flex items-center gap-3 min-w-0">
                  ...
                  <span className="text-sm text-slate-700 truncate">{rel.title}</span>
                </div>
      ...
      {deleteEvidenceId !== null && (
        <div className="fixed inset-0 bg-black/40 flex items-center justify-center z-50 p-4">
          <div className="bg-white rounded-xl shadow-xl p-6 max-w-sm w-full">
            <h3 className="font-semibold text-slate-900 mb-2">Delete Evidence File</h3>
